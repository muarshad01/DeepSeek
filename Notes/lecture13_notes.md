me is Dr. Raj Dandkar. I graduated with a PhD in machine learning from MIT in 2022 and
0:08
...
2:08
scratch series. In this lecture, we are going to code the multi head latent
2:13
attention mechanism from scratch. In the previous lecture, we started
2:19
taking a look at the latent attention mechanism which deepseek introduced into
2:24
their architecture. So let me quickly revise the most fundamental version of
2:30
this mechanism. There are of course advanced versions of the multiad latent attention which deep also introduced and
2:38
we are going to cover that later in the course. But currently the main focus is to
2:43
understand the most basic version of multi head latent attention. And in the previous lecture we saw that this is
2:50
what happens in that version. We have an input matrix which corresponds to the
2:57
input sequence and remember during inference we are going to predict one
3:02
new token at a time. So whenever we are having lectures with respect to latent
3:07
attention always think from the perspective of LLM inference. So we have four tokens let's
3:15
say and these four tokens each of them have an embedding dimension of eight. So
3:20
what happens in latent attention is that as the first step we are going to project this input embedding matrix into
3:27
a latent space. To do that projection, we are going to multiply the input embedding matrix with this DKV matrix,
3:35
which is the down projection KV matrix, which takes my input embedding from an
3:40
8dimensional space and projects it to a lower dimensional space. And in this case, I'm assuming this lower
3:46
dimensional space to be fourdimensional. The product gives me this matrix which I'm calling CKV over
3:53
here. And it turns out that in latent attention all you need to do is just cache this one matrix. You don't have to
4:01
cache the keys matrix separately. You don't have to cache the values matrix either. You just cache this one matrix.
4:08
That's all. And that reduces your KV cache memory size by a lot.
4:14
Once you get this latent matrix which is cast, you multiply it with W UK which is
4:20
the keys of projection matrix and W UV which is the values of projection matrix
4:26
and then you get the keys and the values. Rest of the mechanism follows very similar to what happens in the
4:33
multi head attention block. But there are two key things to truly understand the latent attention mechanism. First is
4:40
this latent latent matrix CKV and second is what we called as the absorption
4:46
trick. So you might be wondering what really did we achieve by adding this one
4:52
more matrix over here. To truly understand that there is a very beautiful mathematical trick in which
4:58
this WQ and W U K will be merged together. So traditionally the queries
5:04
...
vector is calculated by multiplying the input with WQ right but as you will see we are going to merge WQ with W K so
5:12
that we can only cache so that we'll only need to cache CV and this is how it happens. So Q is
5:20
equal to X * W UQ WQ. CV is X * W D KV. We saw this in
5:28
this figure right now. And then the keys matrix is just this CKV multiplied by W
5:33
U K. And the values matrix is just CV multiplied by WUV. So wherever we have
5:40
CV, we can put in X into W DKV. So the keys matrix can be represented like
5:47
this. and the values matrix can be represented like this. The absorption trick comes into picture when we
5:53
calculate the attention scores. So to calculate the attention scores, what we are going to do is that we are going to
6:00
multiply we are going to multiply queries with the case transpose which is X into WQ multiplied by all of these
6:08
values. Now what you can do is that you can merge W UK transpose with WQ. This
6:14
was the merging I was talking about W Q with W UK. You merge these two
6:22
um into one matrix. So this will be WQ into W UK transpose. And then what
6:28
remains in the other bracket is X into WD KV. And remember we are caching this
6:34
because X into WD KV leads to CKV which is the only latent matrix I need to cache. So my absorbed query now becomes
6:43
X * WQ * W UK transpose instead of just multiplying X with
6:49
WQ. And remember WQ and W UK transpose are fixed at training time. We only need
6:56
to compute them once. So this absorption trick is the second main understanding
7:01
which you need to understand what actually happens in latent attention. What happens is that we project the
7:07
input into a lower dimensional space and the computations work out such that we only need to compute this or we only
7:14
need to cache this latent matrix. All other things take care of themselves through this absorption trick. So we get
7:21
this absorbed query and then we multiply it with this cast matrix that gives us
7:27
the attention scores. Then to get the attention weights we just do the scaling
7:33
soft max and then to get the context vector matrix we multiply the attention weights with the values matrix and
7:40
remember the values is also x * w dv * wuv. So the value matrix can be written
7:47
as X * W D KV * W UV and from the context vector matrix to
7:55
get my logix matrix which is the matrix through which we'll eventually get our next local prediction we just multiply
8:02
with the output projection matrix W. So here also X * W DKV is cached and W UV *
8:10
WO is fixed at training time. So we only need to compute once. What I'm trying to
8:16
show over here is that even if we just cache this latent matrix, we can still
8:22
get my logits matrix and that can help me get my next token prediction. So I
8:28
just need to cache one matrix and that's my latent matrix. Even for my attention scores calculation, I just need one
8:35
cache. And for my values matrix and the eventual context vector matrix and the
8:40
logics matrix computation, that same cache can be used. And that cache is called as the
8:47
latent KV cache, which is the input multiplied by WTKV. So when a new token comes in, what
8:53
we do is that we first compute the absorbed query. absorb query which is X
8:59
* WQ * W U Krpose. When a new token comes in, we first find this. So if a
9:07
new token is bright, let's say it's an 8 dimensional vector, we multiply it with WQ multiplied by WK transpose and that
9:14
gives us the absorbed query vector for Bright. And to get the attention scores, we simply need to multiply this absorbed
9:20
query vector with my cache. So I first get the new cache vector which is Xbrite
9:28
which is the new input embedding multiplied with W DKV that's my new KV
9:34
cache and then I append it to the earlier cache value. So that's my updated KV cache and then I just
9:41
multiply my absorb query vector with the updated KV cache and then I get the
9:46
attention scores for Bright. Once I have the attention scores for bright simultaneously what I can do is that I
9:53
can use my same updated KV cache I can multiply it with Wuv and I get my values
9:59
matrix. Then I multiply the attention weights multiplied with the values
10:05
matrix and that gives me the context vector for bride which is my latest token and that eventually helps me
10:12
predict the next token. So here you see the only thing we are caching is this latent KV cache. Even
10:20
for the attention scores calculation we are just caching this and even for the values matrix calculation which gives us
10:27
the context vector we only need to use the same cache. We don't need to cache the keys and the values separately. So
10:34
