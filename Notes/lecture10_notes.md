...
* Consider $n_{heads}=4 \rightarrow 1$:
  ...
  * $W_{K_1}=W_{K_2}=W_{K_3}=W_{K_4} \longrightarrow  K_1=K_2=K_3=K_4=K$ (Cache only one K)
  * $W_{V_1}=W_{V_2}=W_{V_3}=W_{V_4} \longrightarrow  V_1=V_2=V_3=V_4=V$ (Cache only one V)

  * $A_1(Q_1 \times K^T) \neq A_2(Q_2 \times K^T) \neq A_3(Q_3 \times K^T) \neq A_4(Q_4 \times K^T)$ (Attention Scores)
  * $\text{softmax}(A_1) \neq \text{softmax}(A_2) \neq \text{softmax}(A_3) \neq \text{softmax}(A_4)$ (Attention Weights)
...
#### Disadvantage of MQA
* We still capture multiple perspective, however, the ablity to capture multiple perspectives has reduced. The only tunable parameter is Queries ($Q$) as K and V are same for ALL heads.
