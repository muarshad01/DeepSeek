...
2:08
scratch series for a quick recap here is what we have covered so far we are on our
2:16
journey to understand multi-head latent attention and up till now we have understood self attention we covered
2:23
this module from Basics and also the coding part of this we understood causal
2:28
attention in the previous lecture and today we are going to understand the fundamentals of multi-head attention in
2:36
the next lecture we are going to code multi-head attention but today the main
2:41
thing which I want to convey to you is what what is the real need for having
2:47
this multi-head attention and why can't we just stick with causal attention
2:52
let's say the more I read in literature and the more resources I have seen very very
3:00
few people have explained what is the real need for having multiple heads in
3:05
attention so without going into the code or without directly jumping into the code first it's very important for us to
3:13
develop a very strong intuition of why we have multiple heads so today's
3:19
lecture is titled going from self attention to multi-head attention and I'm going to try to explain this to you
3:25
in a very intuitive manner directly from the first principles so let's get started with today's lecture as we have
3:32
seen before the main goal of the self attention mechanism is essentially to take input embeddings and to transform
3:39
them into something which is called as context embeddings remember that input embeddings on their own do not contain
3:46
any information about neighbors so for example if I look at a single token in a sentence input embedding captures the
3:53
semantic meaning of that token and its position but it does not capture how that token relates to the other tokens
4:00
in the sequence and that's the additional mechanism which self attention brings into the
4:06
picture um the context Vector which is the main outcome of the self attention
4:12
mechanism contains information of how a given token attends to all its
4:17
neighboring tokens and to go from the input embedding Matrix to the context
4:22
embedding Matrix we have essentially four parts the first part is transformation to query key value Matrix
4:29
pris second part is calculation of the attention scores third part is
4:34
calculation of the attention weights and fourth part is the context Vector Matrix calculation so let's quickly see these
4:41
four parts in action right now so let me take you to
4:47
that um let me take you to that part of the notebook where we did those
4:52
calculations here it is so here's the input embedding Matrix which we have
4:57
there are 1 2 3 4 5 tokens and each token is essentially an eight dimensional Vector in the first step
5:05
what we do is that we multiply this input embedding Matrix with trainable query key and value weight matrices and
5:12
then we get this query Vector Matrix the key Vector Matrix and the value Vector Matrix after this point we forget about
5:20
the input embedding Matrix completely and only operate on these queries keys and the values that's step number one
5:27
step number two is we take a product of queries multiplied by the
5:32
keys transposed and that essentially gives us the attention scores so every row of this corresponds
5:40
to the attention score of that particular token so for example the second row of this Matrix corresponds to
5:46
the attention scores for next and how next actually relates to
5:51
the next day is bright
6:00
so these five attention scores are mentioned in this or are calculated in this second row similarly all other rows
6:07
have attention scores of that particular token the main drawback of attention
6:13
scores is that the rows don't sum up to one so we cannot make intuitive claims
6:19
that uh when next is the query give 10% importance to the give 50% importance to
6:25
next Etc so to do that we have to normalize the attention scores and we have two steps in the
6:31
normalization first what we do is that we take the attention scores and we divide them by the square root of the
6:37
keys Dimension this is done so that the variance of the qu queries multiplied by
6:42
the keys transpose that stays equal to one and the variance does not become too
6:47
large and then we apply the soft Max function the soft Max essentially ensures that when you look at a
6:53
particular row all the entries of that row will sum up to one so this is normal self attention
7:01
where we look for a given token we look forward as well as backward in the causal attention mechanism what we do is
7:08
that all the elements essentially about this diagonal all of these
7:14
elements are essentially put to zero because for a given token we should not
7:19
have access to all the tokens which come after that particular token so all of these elements which I marked in the
7:25
green color right now they are effectively masked to zero that's what happens s in the causal attention
7:31
mechanism and then we can apply Dropout to the attention weight so that we
7:36
randomly mask out some elements to be zero this is to improve the generalization performance and to
7:42
prevent over fitting the last step which is done here is that we take the attention weights Matrix and we multiply
7:49
with the value Matrix to give us the context Vector Matrix and here you can see that every row of the context Vector
7:56
corresponds to my token so the first row is the context Vector for the the second
8:01
row is the context Vector for next Etc and if you look at the visualization
8:06
of the context Vector you'll see that the context Vector is much richer because it takes into account how much
8:13
attention is paid to all the nearby or the neighboring tokens of next for example whereas the input embedding does
8:20
not have that richness in information about the neighboring tokens so this was in a nutshell the
8:26
self attention mechanism and the causal attention mechan mechanism also remember that when we saw the causal attention
8:33
mechanism what we essentially did was we we put the all the elements of the
8:39
attention weights Above This diagonal to be equal to zero and then after that we can also add Dropout so we can further
8:47
randomly turn off certain attention weights to zero that was causal attention Okay so with that let's get
8:54
started with today's lecture in which we will motivate why do we need to go from self attention to multi-ad attention so
9:01
first remember that self attention is awesome and self attention has a lot of advantages because we finally start
9:09
including or encoding information about the context which is present in the given sentence without self attention
9:16
mechanism language models would not be as good as understanding the meaning of sentences as they are today but for all
9:25
the advantages of self attention self attention mechanisms come with one very
9:31
major problem and multi attention solves that
9:36
problem so let's see what this problem with the self attention mechanism is and
9:41
...
10:04
a woman with a brush now try to think about what do you
10:10
think this sentence means you can pause the video for a while and think about the interpretation
10:16
of this sentence okay so if you have thought for some time you will have realized that
10:23
the reason I have chosen this sentence is that it has two interpretations the first inter
10:29
interpretation is that the artist so let's say if this is the artist the artist has painted the
10:35
portrait of this woman using a brush so the artist painted the portrait
10:43
of a woman with a brush which means that the artist the artist had a brush in their hand and with that the artist
10:50
painted the portrait of a woman that's the first interpretation the second interpretation is that the artist
10:56
painted the portrait of a woman with a brush so the painting is of a woman who
11:02
has a brush in their hand and the artist has painted
11:09
this so you see the difference in the first case the painting is that of a woman who does not have a brush in her
11:16
hand the artist is painting the portrait with a brush but in the second case it's
11:22
the painting or the portrait of a woman with a brush in their hand so this is the painting so this is a woman with a
11:29
brush in her hand and the artist has painted this so the first is a painting
11:34
painting a woman with a brush and the second is painting of a woman with a
11:39
brush right so these are the two interpretations
11:44
of this sentence the problem of the self attention mechanism comes into the
11:51
picture over here so what I want to now do is that I want you to focus on two
11:57
attention scores matrices and which I'm I'm going to uh put them together side
12:03
by side over here now so take a look at the attention score Matrix number one
12:12
and so take a look at attention score number Matrix number one and Matrix number two and what I'm also going to do
12:18
is that I'm going to bring these corresponding images below
12:24
it okay so this is the first image and then I'm also going to bring
12:30
the second image
12:37
now here Ive brought the second image now and let me just rub this
12:43
Arrow okay now I want you to focus on these two attention score matrices okay
12:49
the first attention score Matrix which is which I marked here as number one it
12:55
corresponds to this this image right and why is that because focus on the woman
13:02
if the woman is my query the circles which I've marked in
13:08
the red ear correspond to the highest or the strongest attention scores so you can focus on those red circles for now
13:15
so here we have a red circle between woman and a portrait since it's a portrait of a woman the attention score
13:21
between woman and portrait is quite high that is understandable whereas if you see over
13:27
here in the the second in the second attention scores Matrix the attention
13:33
score between woman and the portrait is still high that is understandable
13:38
because here also we have a portrait of a woman but notice this red color over
13:44
here the attention score between woman and the brush is also very high and why
13:50
is the attention score between woman and the brush also High because in the second interpretation the woman is
13:56
holding a brush in her hand that is the difference between the atten
14:03
this the atten this Row in the first attention score Matrix and this Row in the second attention score Matrix in the
14:10
second attention scores the attention score between woman and the brush is quite High whereas that's not the case
14:16
in the first attention score because in the first attention score Matrix the woman is not holding a brush in her hand
14:23
the second distinction which we see is when we look at uh um when we look at essentially
14:32
artist so when we look at artist in the first Matrix the artist has a high
14:37
attention score with painted and the artist has a high attention score with
14:44
brush why because the artist is holding a brush in her hand in the first
14:49
interpretation whereas if you look at the second interpretation um if you look at the
14:55
second interpretation the artist is actually not holding a brush in their
15:02
hand and that's why the attention score between artist and the brush is actually
15:07
not very high that's the second difference between these two attention score
15:13
matrices and the third difference between these two is when you look at
15:20
brush so when you look at brush you'll see that the attention if brushes the query it has high attention scores with
15:28
artist because the artist is holding the brush and it has high attention scores
15:35
...
15:55
a woman with a brush and brush also has a high ATT attention score with
16:01
woman why because the woman has a brush in her hand so brush has a high attention score with portrait and woman
16:08
in this case whereas in this case brush has a high attention score with
16:14
artist so I hope I have convinced you now that in these two interpretations of
16:20
the sentence the attention scores matrics are completely different from each other right there can be two
16:26
attention score matrices based on on the interpretation which you have for the
16:31
sentence now the main problem with the self attention mechanism is that the self attention mechanism can only
16:38
capture a single perspective in a given input sequence it cannot capture multiple perspectives so if you are
16:46
looking at this sentence the self attention mechanism will either have this attention scores Matrix or it will
16:52
have this attention scores Matrix and consequently the context Vector will either be based on the first attention
16:58
score Matrix or the second attention score Matrix so it cannot take into account both of these perspectives at
17:04
the same time and that's one major limitation of the self attention mechanism that if you are given a piece
17:11
of paragraph or piece of sentences self attention can only capture one
17:16
perspective so if you think about it more deeply why would you want to capture multiple perspectives in a
17:22
sentence the reason you would want to capture multiple perspectives in a sentence or paragraph is that some times
17:29
it is difficult to understand um what the paragraph represents in just one
17:35
form or meaning one paragraph might have different angles right one paragraph
17:42
might be expressed in multiple different ways and when we design our language
17:48
model we want our language model to be knowledgeable of multiple different perspectives present within a paragraph
17:55
an example here is that let's say we have a huge paragraph right and within that paragraph We have a sentence which
18:02
reads like this the government should regulate Free Speech now if you look at this sentence
18:09
the government should regulate Free Speech the first perspective should be the government should impose
18:15
restrictions on free speech so free speech should be curtailed that's the first perspective
18:21
the second perspective should be the government should protect and preserve Free Speech it's not really clear from
18:28
this sentence which perspective is um this paragraph referring to so it just
18:33
say regulate free speech so regulating Free Speech can either mean that Free
18:38
Speech should be curtailed or Free Speech should be protected and
18:43
preserved what does this line really mean so if we use the self attention
18:49
mechanism again it will assign only one perspective to this but we actually want
18:55
the attention mechanism to capture both of the persp perspectives which this sentence can
19:02
include and the advantage of capturing both perspectives is that our model just becomes more richer in its understanding
19:09
so when a user asks to summarize this document we can summarize both the perspectives whereas if we just use a
19:16
self attention mechanism and a user asks to summarize this document we will only summarize one perspective and that is
19:23
not good so we want to overcome this limit
19:28
itation of self attention mechanism so essentially what we want to do is that we want to have provision in our model
19:36
so that we can have or we can capture different perspectives in my paragraph
19:43
So if one self attention mechanism can only capture one perspective can we somehow extend the
19:49
self attention mechanism so that it can capture multiple perspectives so if one self attention
19:56
mechanism can essentially just capture one perspective within the same architecture
20:04
what if I have multiple self attention mechanisms what if I have multiple self
20:09
attention mechanisms so if one self attention mechanism can capture one perspective multiple self attention
20:16
mechanisms can capture multiple perspective so if let's say one self
20:24
attention mechanism gives me one context Vector Matrix another self attention mechanism gives me another context
20:30
Vector Matrix a third self attention mechanism gives me a third context Vector Matrix I'll have these multiple
20:36
context Vector matrices Each of which capture different perspectives and then I'll just merge them together so that
20:43
